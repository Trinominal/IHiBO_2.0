...
pragma solidity ^0.8.28; // updated to a more recent version


contract BalanceBasedDetachment {
    ...
    struct Weight {
        uint256 COG_id; // context option ground id of the weight
        uint256 justifyingWeight;
        uint256 requiringWeight;
    }

    mapping(Issue => mapping(Option => mapping(Option => mapping(Reason => Weight)))) weightSystem;

    uint256[] issueIDs;
    mapping(uint256 => Issue) issues;

    address[] agents;
    uint256[] COG_ids; // context option ground ids
    mapping(address => mapping(Weight => uint256)) reputations;
    mapping(address => mapping(Weight => Weight)) votes;//agentsArguments, sources

    // uint256[] contextIDs;
    // mapping(uint256 => uint256) contexts;


    event deonticStatus(Option option);


    constructor() public {
        issueIDs.insert(uint256(0));
        Issue storage issue = issues[0];
        issue.optinoIDs.push("do nothing");
        Option storage option = issue.options[0];
        option.id = "do nothing";
        option.deonticStatus = "neutral";
        issue.reasonIDs.push("no reason");
        Reason storage reason = issue.reasons[0];
        reason.id = "no reason";
        reason.ground = "no ground";
        issue.contextID = 0;
    }
...
// ------------- interaction functions ---------------------------------------
    ...
    function setReputation(uint256 rep, address subject) 
        public
    {
        assert(msg.sender!=subject); // you can not set your own reputation
    ...
    function getReputation(address subject) 
        ...
        returns (uint256 rep)
    {
        rep = reputations[subject];
    }

    function setWeight(Issue issue, Option option1, Option option2, uint256 reasonID, uint256 newJWeight, uint256 newRWeight)
        public
    {
        assert(reasonID != 0);

        weightSystem[issue][option1][option2][reasonID].justifyingWeight = newJWeight;
        weightSystem[issue][option1][option2][reasonID].requiringWeight = newRWeight;
    }

    function getWeight(Issue issue, Option option1, Option option2, uint256 reasonID)
        public
        view
        returns (Weight weight)
    {
        weight = weightSystem[issue][option1][option2][reasonID];
    }
