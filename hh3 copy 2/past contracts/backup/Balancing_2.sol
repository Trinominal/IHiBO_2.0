...
contract Balancing {
    ...
    struct Weighing {
        SimpleSet.Set reasonsIDs;
        mapping(uint256 => Reason) reasons;
        string option;
        string output;
    }

    SimpleSet.Set weighingIDs;
    mapping(uint256 => Weighing) weighings;

    mapping(address => SimpleSet.Set) sourcesReasons;//agentsArguments, sources
        ...
        returns (int256 reason)
    {
        ...
        for (uint256 j = 1; j < weighing.reasonsIDs.count() + 1; j++) {
            ...
            compareStrings(weighing.reasons[reasonID].polarity, polarity)) {
                ...
                SimpleSet.Set storage source = sources[ msg.sender];
