...
import { ArgumentationModule } from "../ignition/modules/ArgumentationModule";
import { expect } from "chai";

describe("Argumentation with Viem", function () {
  let viem: Awaited<ReturnType<typeof network.connect>>["viem"];
  let publicClient: Awaited<ReturnType<typeof viem.getPublicClient>>;
  let walletClients: Awaited<ReturnType<typeof viem.getWalletClients>>;
  let alpha: typeof walletClients[0];
  let beta: typeof walletClients[1];
  let gamma: typeof walletClients[2];
  let contract: any;
  ...
  beforeEach(async () => {
    ...
    [alpha, beta, gamma] = walletClients;

    const deployment = await viem.deploy(ArgumentationModule);
    contract = deployment.argumentation;
  });

  it("graph 1, IHiBO original", async () => {
    await alpha.writeContract({
      ...contract,
      functionName: "insertArgument",
      args: ["a"],
    });

    await beta.writeContract({
      ...contract,
      functionName: "insertArgument",
      args: ["b"],
    });

    await gamma.writeContract({
      ...contract,
      functionName: "insertArgument",
      args: ["c"],
    });
    ...
    const graph1 = await publicClient.readContract({
      ...contract,
    ...
    await alpha.writeContract({
      ...contract,
      functionName: "pafReductionToAfPr1",
      args: [],
    });
    ...
    const txHash = await alpha.writeContract({
      ...contract,
    ...
    for (const log of receipt.logs) {
      try {
        ...
        if (decoded.eventName === "PreferredExtensions") {
          console.log("PreferredExtensions:", decoded.args);
        }
      } catch {
        // Not the right event
      }
...
// import { ArgumentationModule } from "../ignition/modules/ArgumentationModule.js";
// import { expect } from "chai";
// import { Signer } from "ethers";

// describe("Argumentation", function () {
//   let alpha: Signer;
//   let beta: Signer;
//   let gamma: Signer;
//   let sc: any;
...
//   beforeEach(async () => {
//     const deployment = await ignition.deploy(ArgumentationModule);
//     sc = deployment.argumentation;
...
//     await scAlpha.insertArgument("a");
//     await scBeta.insertArgument("b");
//     await scGamma.insertArgument("c");
...
//     await sc.pafReductionToAfPr1();
//     const r1 = await sc.getGraph(2);
//     console.log("Reduction 1:", r1);

//     await sc.pafReductionToAfPr3();
...
//         if (parsed.name === "PreferredExtensions") {
//           console.log("PreferredExtensions:", parsed.args);
//         }
//       } catch (e) {
//         // Not the right event
//       }
...
//     deployed = await Argumentation.deploy(); // Deploys the contract
...
describe('Argumentation 1', function () {
  ...
  beforeEach(async function () {
    ...
    const deployed = await Argumentation.deploy(); // Deploys the contract
